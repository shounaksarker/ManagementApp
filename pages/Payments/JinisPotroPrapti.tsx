...
const JinisPotroPrapti = () => {
  const [payment, setPayment] = useState<Payment[]>();
  const [totalAmount, setTotalAmount] = useState(0);
  const [pay, setPay] = useState({
    type: "JinisPotroPrapti",
    date: "",
    amount: "",
  });
  ...
  const mount = async () => {
    await axios
      .get("/api/Payments/getPayments/JinisPotroPrapti")
      .then((res) => {
        setPayment(res.data);
        let arr: number[] = [];
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="mb-4 alert alert-primary">Receipt of goods</h3>
        <Row className="row">
...
export default JinisPotroPrapti;
